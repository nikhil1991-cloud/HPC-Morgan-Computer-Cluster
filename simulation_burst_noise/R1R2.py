...
path_output = '/mnt/gpfs2_4m/scratch/naj222/dir1/output/simulation_burst_noise/'
...
for f in range (0,len(fraction_array)):
 ...
 M_FSIM = np.zeros((5,100))
 BC03_FSIM = np.zeros((5,100))
 DELTATF_ALL = np.zeros((5,100))
 MASS_ALL = np.zeros(5)
 T1 = np.zeros(5)
 T2 = np.zeros(5)
 T3 = np.zeros(5)
 ...
 for eTau in range (0,len(e_folding_time)):
    e_fold_tau = 1
    qe_fold_tau = e_folding_time[eTau]
 ...
 hdu.writeto(path_store+'R1R2_simb_'+str(fraction)+'.fits')
