...
for f in range (0,len(fraction_array)):
 ...
 for e in range (0,len(e_folding_time)):
  ...
  #Initialize array
  reader = emcee.backends.HDFBackend(path_output+'_'+str(qe_fold_tau)+'_'+'sigma_'+str(fraction))
  ...
  stat=0
  for stat in range (0,3):
     plt.plot(diff_lick_ang[:,stat]/Lick_indices_error[~mag_index],label=statistics[stat],c=stat_c[stat],linewidth=lw)
  num=0
  ...
  plt.ylabel('$\\Delta$Lick/$\sigma_{Lick}$ [Ang]',fontsize=ft_12,labelpad=pd, fontweight='bold')
...
#  plt.xticklabels(Indices_color,rotation=20,fontsize=ft_12-80)
  ...
  fig.savefig(path_plots+'sfr_obs/'+str(qe_fold_tau)+'sigma_'+str(fraction)+'.png')
  plt.close()
 
 
  ALL_PAR = np.c_[R1_all,R2_all,R3_all]
  ALL_S = np.c_[s1_all,s2_all,s3_all,s4_all]
  f1_SFH = nflat_SFH[:,:-3]
  f2_SFH = np.c_[ALL_PAR,nflat_SFH[:,-3:]]
  Median_PAR = np.c_[R1_median,R2_median,R3_median]
  MLE_PAR = np.c_[R1_mle,R2_mle,R3_mle]
  Median_S =  np.c_[s1_median,s2_median,s3_median,s4_median]
  MLE_S = np.c_[s1_mle,s2_mle,s3_mle,s4_mle]
  Mode_PAR = np.c_[R1_mode,R2_mode,R3_mode]
  ...
  data_f2 = np.array([R1_sim,R2_sim,R3_sim,0.5,0.05,0.0])
 ...
 # Loop over the diagonal
  ...
  fig.savefig(path_plots+'corner/'+str(qe_fold_tau)+'sigma_'+str(fraction)+'_c1.png')
  plt.close()
 ...
 # Loop over the diagonal
  ...
  for ax in fig.get_axes():
        ax.tick_params(axis='both', labelsize=15)
  fig.savefig(path_plots+'corner/'+str(qe_fold_tau)+'sigma_'+str(fraction)+'_c2.png')
  plt.close()
 ...
 # Loop over the diagonal
  ...
  fig.savefig(path_plots+'corner/'+str(qe_fold_tau)+'sigma_'+str(fraction)+'_c3.png')
  plt.close()
