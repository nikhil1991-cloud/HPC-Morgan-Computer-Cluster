...
def log_likelihood(Initial_theta,t_age,CI,CR,CB,D4,Photo,PhotoAB,L_R,L_B,L_I,D_I,EBV_I,Metal_I,D_lmd,Lick_indices_data,Dn4000_data,Mags_data,Dn4000_error,Lick_indices_error,Mags_error,nsa_mass,DATA_4SSP,good_lick):
    ...
    CHI_LICK = np.sum((((Lick_indices_data - Model_LICK_I)/(Lick_indices_error))**2)[good_lick])
    ...
    CHI_COL = np.sum(((Data_color - Model_color)/Color_error)**2)
...
def log_probability(Initial_theta,t_age,CI,CR,CB,D4,Photo,PhotoAB,L_R,L_B,L_I,D_I,EBV_I,Metal_I,D_lmd,Lick_indices_data,Dn4000_data,Mags_data,Dn4000_error,Lick_indices_error,Mags_error,nsa_mass,DATA_4SSP,good_lick):
    lp = log_prior(Initial_theta,nsa_mass)
    if not np.isfinite(lp):
        return -np.inf
    lprob = lp + log_likelihood(Initial_theta,t_age,CI,CR,CB,D4,Photo,PhotoAB,L_R,L_B,L_I,D_I,EBV_I,Metal_I,D_lmd,Lick_indices_data,Dn4000_data,Mags_data,Dn4000_error,Lick_indices_error,Mags_error,nsa_mass,DATA_4SSP,good_lick)
    return lprob
