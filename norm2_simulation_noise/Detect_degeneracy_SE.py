...
import math
import os
#os.chdir('/Users/nikhil/code/Paper_plots') #Comment on cluster
...
for f in range (0,len(fraction_array)):
 ...
 for e in range (0,len(e_folding_time)):
  ...
  reader = emcee.backends.HDFBackend(path_output+'_'+str(e_fold_tau)+'_sigma_'+str(fraction))
  ...
  Residue = (SFR_bestfit.T - SFR)
  l_idx = np.where((ll>3900)&(ll<4900))[0]
  ...
  ax1.axvline(3950,linewidth=lw,c='violet',label='CaHK')
  ax1.axvline(4000,linewidth=lw,c='blue',label='D$_{n}$(4000)')
  ax1.axvline(4102,linewidth=lw,c='green',label='H$\\delta_{A}$')
  ax1.axvline(4862,linewidth=lw,c='yellow',label='H$\\beta$')
  ax1.set_ylabel('Log[f$_{\\lambda}$($L_{\odot}$/$\AA^{-1}$)]',fontsize=ft_12+40,labelpad=pad_space+5)
  ax1.set_xlim([3900,4900])
  ...
  ax2.set_ylabel('Log[f$_{\lambda input}$/f$_{\lambda output}$]',fontsize=ft_12+40,labelpad=pad_space+5)
  ax2.axhline(y=0,linewidth=lw,c='grey')
  ax2.set_ylim([res_min,res_max])
  ax2.set_xlim([3900,4900])
  ax2.minorticks_on()
  ...
  fig.savefig(path_plots+'spectra/'+str(e_fold_tau)+'_'+str(fraction)+'spectra.png')
  plt.close()
  ...
  ax2.hist(R2_sample[:-1],bins='auto',edgecolor='r',facecolor='None',linewidth=lw-2)
  ax2.axvline(R2_sim,linewidth=lw+4,c='k',label='Input R2')
  ax2.legend(loc='upper left',fontsize=ft_22-20,frameon=False)
  ax2.set_xlabel('R2',fontsize=ft_22)
  ax2.minorticks_on()
  ax2.tick_params(which='major',length=ltw,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='x',bottom=True,top=True,left=True,right=True)
  ax2.tick_params(which='minor',length=ltwm,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='x',bottom=True,top=True,left=True,right=True)
  ax2.tick_params(which='major',length=ltw,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='y',bottom=True,top=True,left=True,right=True)
  ...
  ax3.hist(R3_sample[:-1],bins='auto',edgecolor='r',facecolor='None',linewidth=lw-2)
  ax3.axvline(R3_sim,linewidth=lw+4,c='k',label='Input R3')
  ax3.legend(loc='upper left',fontsize=ft_22-20,frameon=False)
  ax3.minorticks_on()
  ax3.tick_params(which='major',length=ltw,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='x',bottom=True,top=True,left=True,right=True)
  ax3.tick_params(which='minor',length=ltwm,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='x',bottom=True,top=True,left=True,right=True)
  ax3.tick_params(which='major',length=ltw,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='y',bottom=True,top=True,left=True,right=True)
  ax3.tick_params(which='minor',length=ltwm,width=lw,direction='in',labelsize=ft_1,pad=pad_space,axis='y',bottom=True,top=True,left=True,right=True)
  for axis in ['top','bottom','left','right']:
  ...
  fig.savefig(path_plots+'spectra/'+str(e_fold_tau)+'_'+str(fraction)+'spectra_pars.png')
  plt.close()
