...
path_output = '/mnt/gpfs2_4m/scratch/naj222/dir1/output/norm2_mpi_all_lick/'
...
for i in range (0,np.shape(Line0)[0]):
    ...
    HA_HB = hdu[26].data[18][-1]/hdu[26].data[11][-1]
    HA_sigma = 0.5*np.sqrt(hdu[26].data[18][-1])
    HB_sigma = 0.5*np.sqrt(hdu[26].data[11][-1])
    HA_HB_sigma = np.sqrt(((hdu[26].data[18][-1]*HB_sigma)**2 + (hdu[26].data[11][-1]*HA_sigma)**2)/(hdu[26].data[11][-1])**4)
...
hdu.writeto(path_store+'R1R2_global.fits')
